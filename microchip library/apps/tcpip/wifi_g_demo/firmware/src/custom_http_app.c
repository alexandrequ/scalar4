    ...
    while (curHTTP.byteCount) {
        ...
        if (!strcmppgm2ram((char *) curHTTP.data, "sec")) {
            ...
            if (!strcmppgm2ram((char *) security_type, "no")) {
                ...
                CFGCXT.securityMode = WF_SECURITY_WPS_PUSH_BUTTON;
                printf("WPS Push-Button");
            } else if (!strcmppgm2ram((char *) security_type, "wps_pin")) {
                CFGCXT.securityMode = WF_SECURITY_WPS_PIN;
                printf("WPS PIN");
...
{
    TCPPutArray(sktHTTP, AppConfig.ssid, AppConfig.ssidLen);
}
...
void HTTPPrint_prevWLAN(void)
{
    if (CFGCXT.prevNetworkType == WF_INFRASTRUCTURE)
        ...
        TCPPutROMString(sktHTTP, (ROM uint8_t *) "SoftAP (BSS)");
    else if (CFGCXT.prevNetworkType == WF_ADHOC)
        TCPPutROMString(sktHTTP, (ROM uint8_t *) "Adhoc (IBSS)");
    else if (CFGCXT.prevNetworkType == WF_P2P)
        TCPPutROMString(sktHTTP, (ROM uint8_t *) "WiFi Direct P2P (BSS)");
    else
        TCPPutROMString(sktHTTP, (ROM uint8_t *) "Unknown BSS");
}
