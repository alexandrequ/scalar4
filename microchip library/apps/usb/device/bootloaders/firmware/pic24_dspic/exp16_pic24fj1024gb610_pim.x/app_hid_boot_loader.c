...
/******************************************************************************
 * NOTE:  This code currently only supports bootloading/programming operations 
 * while the device is configured to operate in "single partition" mode.
 ******************************************************************************/
    ...
    else //(BootState must be in NotIdleState)
    {
        switch(PacketFromPC.Command)
        {
            ...
            {                
                ...
                for(ErasePageTracker = BOOT_CONFIG_USER_MEMORY_ERASE_PAGE_START; ErasePageTracker < MaxPageToErase; ErasePageTracker++)
                {
...
static void WriteFlashSubBlock(uint16_t unlockKey)
{
    ...
    while(BufferedDataIndex != 0)		//While data is still in the buffer.
    {
        ...
        if(Address.Val >= BOOT_CONFIG_USER_MEMORY_START_ADDRESS)
        {
            ...
            //Check if programming the FSIGN config word, that the "Reserved: Maintain as 0" bit is actually being
            //programmed to 0.  This may not necessarily be the case if the .hex file doesn't contain anything at 
            //this address, and in this scenario, we should force it to 0 to obey the datasheet.
            if(Address.Val == BOOT_MEMORY_CONFIG_FSIGN_ADDRESS)
            {
                __builtin_tblwtl((uint16_t)(NVM_WRITE_LATCH_STARTING_ADDRESS+0), ProgrammingBuffer[i + 0] & 0x7FFF);	//Write the low word to the latch
...
void APP_HIDBootloaderShutdown(void)
{
    ...
    //This reduces chances of application code inappropriately jumping into the bootloader
    //and executing some unintended erase/write operations.
    bootloaderProperInitKey = ~CORRECT_BOOTLOADER_INIT_KEY;
}
