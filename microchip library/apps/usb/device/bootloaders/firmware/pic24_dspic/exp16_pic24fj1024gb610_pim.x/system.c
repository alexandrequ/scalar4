...
void SYSTEM_Initialize( SYSTEM_STATE state )
{
    switch(state)
    {
        case SYSTEM_STATE_USB_START:
            //Switch to alternate interrupt vector table for bootloader mode operation.
            INTCON2bits.AIVTEN = 1;
            //Configure I/O pin as input, for checking if we should start up in bootloader
            //or normal application run mode.
            BUTTON_Enable(BUTTON_USB_DEVICE_HID_CUSTOM);

            //Check if we should jump to the normal application run mode.
            if((BUTTON_IsPressed(BUTTON_USB_DEVICE_HID_CUSTOM) == false) && ((RCON & 0x0083) != 0) && (ReadProgramMemory(0x2000) != 0x00FFFFFF))
            {
                //We need to boot up into the normal application mode (not bootloader firmware update mode).
                //Switch to standard interrupt vector table for non boot mode and jump to the
                //application startup/entry point.
                INTCON2bits.AIVTEN = 0;
                __asm__("goto 0x2000"); //Jump to the start of the application.
            }
            
            //If we get here, that means we should stay in the bootloader.
